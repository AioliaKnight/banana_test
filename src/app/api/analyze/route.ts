...
2. **尺寸估計**：
   - 估計與測量物體的長度（厘米），使用數字，例如18.5
   - 估計與測量物體的粗細/直徑（厘米），使用數字，例如3.2
...
4. **專業評語**：
   ...
   - 使用繁體中文，符合台灣的網路用語，口吻像是在和閨蜜對話，對男性的抱怨與品頭論足,不要使用“妳”，使用“你”。
   - 如果是標準水果（香蕉或小黃瓜），評論其尺寸、形狀、品質等方面。
   - **特別重要**：如果是"other_rod"，請寫一段更加詳細且有趣毒辣的評語，若圖片明確與男性特徵相關,加入女性對男性視角的對話式評論，可以使用「親愛的」等稱呼,不一昧吹捧，要有趣的嘲諷批評。
   - 對於"other_rod"的評語應該富有個性，加點吐槽，口語可以辛辣也可以調皮幽默，可以針對任何觀點，帶有主觀意識給予中肯建議。
   - 評語保持在80-120字之間。
    ...
    try {
      ...
      if (!geminiAnalysis.objectType) {
        ...
        return NextResponse.json(
          { error: { 
              code: 'INVALID_OBJECT', 
              message: '無法辨識小黃瓜或香蕉。請確保圖片中包含完整的小黃瓜或香蕉，並佔據畫面主要部分。目前僅支持分析這兩種水果，其他類似形狀的物體可能被識別為「其他條狀物」類型。' 
            } 
          },
          { status: 400 }
        );
